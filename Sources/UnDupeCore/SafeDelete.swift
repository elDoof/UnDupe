...
public enum SafeDelete {
        ...
        files.map { file in
            guard ProtectedPaths.isDeletable(file.path) else {
                return TrashResult(
                    ...
                    success: false,
                    reclaimedBytes: 0,
                    error: ProtectedPaths.refusalReason(file.path)
                )
            }
            ...
            do {
                ...
                return TrashResult(path: file.path, success: true, reclaimedBytes: file.size, error: nil)
            } catch {
                return TrashResult(
                    path: file.path,
                    success: false,
        ...
        targets.map { target in
            guard ProtectedPaths.isDeletable(target.path) else {
                return TrashResult(
                    ...
                    success: false,
                    reclaimedBytes: 0,
                    error: ProtectedPaths.refusalReason(target.path)
                )
            }
    ...
    public static func deletePermanently(_ targets: [TrashTarget]) -> [TrashResult] {
        targets.map { target in
            ...
            do {
                ...
                return TrashResult(path: target.path, success: true, reclaimedBytes: target.size, error: nil)
            } catch {
                return TrashResult(
                    path: target.path,
                    success: false,
    ...
    /// Moves a single already-vetted path to the Trash. Callers must run the
    /// appropriate `ProtectedPaths` guard *before* calling this.
    private static func trash(path: String, size: Int64) -> TrashResult {
        let url = URL(fileURLWithPath: path)
        do {
            try FileManager.default.trashItem(at: url, resultingItemURL: nil)
            return TrashResult(path: path, success: true, reclaimedBytes: size, error: nil)
        } catch {
            return TrashResult(
                path: path,
                success: false,
